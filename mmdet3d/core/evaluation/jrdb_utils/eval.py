...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    Args:
        gt_annos (dict): Must from get_label_annos() in jrdb_common.py.
        dt_annos (dict): Must from get_label_annos() in jrdb_common.py.
...
def eval_class(gt_annos,
               ...
               compute_aos=False,
               num_parts=200):
    """jrdb eval. support 2d/bev/3d/aos eval. support 0.5:0.05:0.95 coco AP.
...
def jrdb_eval(gt_annos,
    ...
    #                         [0.5, 0.25, 0.25, 0.5, 0.25]])  #세로축 차례로 bbox,bev,3d 하나씩 0넣어보면 알 수 있음
    overlap_0_7 = np.array([[0.7, 1, 0.5, 0.7, 0.5],
                            [0.7, 0.5, 0.5, 0.7, 0.5],
                            [0.7, 1, 0.5, 0.7, 0.5]])
    overlap_0_5 = np.array([[0.7, 1, 0.5, 0.7, 0.5],
                            [0.5, 0.3, 0.25, 0.5, 0.25],
                            [0.5, 1, 0.25, 0.5, 0.25]])
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            ...
            if mAP3d is not None:
                ret_dict[f'JRDB/Overall_3D_{postfix}'] = mAP3d[idx, 0]
            if mAPbev is not None:
                ret_dict[f'JRDB/Overall_BEV_{postfix}'] = mAPbev[idx, 0]
            if mAPbbox is not None:
                ret_dict[f'JRDB/Overall_2D_{postfix}'] = mAPbbox[idx, 0]

    return result, ret_dict


def jrdb_eval_coco_style(gt_annos, dt_annos, current_classes):
    """coco style evaluation of jrdb.
